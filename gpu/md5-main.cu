...
__global__ void testKernel (char* words, md5_byte_t* hash, int* matchIndex) {
  cuPrintf("__global__ testKernel\n");


  cuPrintf("matchIndex: %d\n", *matchIndex);
  ...
  md5_init(&state);
  md5_append(&state, (const md5_byte_t *)word, 2);
  md5_finish(&state, digest);

  int counter = 0;
  for (int di = 0; di < 16; di++) {
    if (digest[di] != hash[di]) {
      continue;
    // cuPrintf("%02x\n", digest[di]);
    } else {
      cuPrintf("We have a match\n");
      counter++;
    }
  }

  if (counter == 16) {
    cuPrintf("setting matchIndex\n");
    *matchIndex = threadIdx.x;
  }
...
int main (int argc, char *argv[]) {
  ...
  break_down_hash(hash_str, hash);

  // Declare words on the host
  char h_words[4][3];
  strcpy(h_words[0], "%%");
  strcpy(h_words[1], "aa");
  strcpy(h_words[2], "bb");
  ...
  // Declare words on the device
  char* d_words;
  cudaMalloc((void**)&d_words, sizeof(h_words));
  cudaMemcpy(d_words, h_words, sizeof(h_words), cudaMemcpyHostToDevice);

  // Declare device hash
  md5_byte_t* d_hash;
  cudaMalloc((void**)&d_hash, sizeof(md5_byte_t) * 16);
  cudaMemcpy(d_hash, hash, sizeof(md5_byte_t) * 16, cudaMemcpyHostToDevice);
  ...
  testKernel<<<1,4>>>(d_words, d_hash, d_index);
  ...
  cudaMemcpy(hash, h_hash, sizeof(md5_byte_t) * 16, cudaMemcpyDeviceToHost);

  int h_matchIndex;
  cudaMemcpy(&h_matchIndex, d_index, sizeof(int), cudaMemcpyDeviceToHost);


  printf("h_matchIndex: %d\n", h_matchIndex);

  // Convert hash to a string
  char result_hash_str[32];
  for (int di = 0; di < 16; di++) {
    sprintf(result_hash_str + di * 2, "%02x", h_hash[di]); 
  }

  printf("result hash_str: %s\n", result_hash_str);
