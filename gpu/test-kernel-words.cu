...
__global__ void testKernel (md5_byte_t* hash_to_break) {
  ...
  for (i_1 = 32; i_1 < 42; i_1++) {
    ...
    // cuPrintf("counter: %d\n", counter);
    if (counter == 16) {
      // cuPrintf("We have a match - first loop\n");
    ...
    for (i_2 = 32; i_2 < 42; i_2++) {
      ...
      // cuPrintf("counter: %d\n", counter);
      if (counter == 16) {
        // cuPrintf("We have a match - second loop\n");
        // cuPrintf("|%c%c%c%c|\n", word[0], word[1], word[2], word[3]);
      }
      for (i_3 = 32; i_3 < 42; i_3++) {
        ...
        // cuPrintf("counter: %d\n", counter);
        if (counter == 16) {
          // cuPrintf("We have a match - third loop\n");
          // cuPrintf("|%c%c%c%c|\n", word[0], word[1], word[2], word[3]);
        }
      } // END Loop 3
    } // END Loop 2
...
int main (int argc, char *argv[]) {
  ...
  // testKernel<<<1,94>>>(d_hash);
  testKernel<<<1,10>>>(d_hash);
